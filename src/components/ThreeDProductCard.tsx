          ...
          <div className="p-3 md:p-6 flex flex-col h-full relative z-20">
            <div className="flex flex-col gap-1.5 mb-2 translate-z-10">
              <div className="flex justify-between items-start w-full">
                {badge ? (
                  <Badge className="bg-white text-black border-none font-black text-[7px] md:text-[8px] tracking-widest px-2 py-0.5 md:px-3 md:py-1.5 rounded-full uppercase shadow-xl">
                    {badge}
                  </Badge>
                ) : <div className="h-4" />}
                {brand && brand !== 'Genérico' && (
                  <span className="text-[7px] md:text-[10px] font-black tracking-widest text-muted-foreground/60 uppercase ml-auto pt-0.5 truncate">
                    {brand}
                  </span>
                )}
              </div>
              ...
              <Badge 
                variant="outline" 
                className="border-white/10 text-muted-foreground font-bold uppercase text-[7px] md:text-[8px] tracking-widest px-2 py-0.5 md:px-3 md:py-1 bg-black/40 backdrop-blur-sm w-fit"
              >
            ...
            <div className="relative h-[180px] md:h-80 w-full flex items-center justify-center overflow-hidden translate-z-20">
              ...
              <div className="relative w-40 h-40 md:w-64 md:h-64 transition-all duration-300 ease-out group-active:scale-[1.03] md:group-hover:scale-[1.05]">
                <Image 
                  src={imageUrl} 
                  alt={name}
                  fill
            ...
            <div className="mt-4 flex flex-col gap-2 translate-z-10">
              <Button 
                className={cn(
                  "w-full pill-button h-11 md:h-12 text-[9px] md:text-[10px] uppercase tracking-widest font-black transition-all",
